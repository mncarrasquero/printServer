...
express()
  ...
  .get("/printGuiaServientrega/:printerName/:idVenta/:url", async function (req, res) {
  ...
  download(url, options, async function(err){
      if(err){
        ...
        console.log( path.join('guias',idVenta+'.pdf') );
          ...
          // };
          const pdfDoc = await PDFDocumentFactory.load(fs.readFileSync(path.resolve('./guias/'+idVenta+'.pdf')));
